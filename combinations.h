...
public:
    call_combine_discontinuous(BidirIter first1,
                               ...
                               D d2,
                               Function& f)
        : f_(f), first1_(first1), last1_(last1), d1_(d1), first2_(first2), last2_(last2), d2_(d2)
    {}
...
template <class BidirIter, class Function>
bool combine_discontinuous3_(BidirIter first1,
                             BidirIter last1,
                             typename std::iterator_traits<BidirIter>::difference_type d1,
                             BidirIter first2,
                             BidirIter last2,
                             ...
                             typename std::iterator_traits<BidirIter>::difference_type d = 0)
{
    ...
    if (d1 == 1) {
        ...
        BidirIter i2 = first2;
        for (D d22 = d2; i2 != last2; ++i2, --d22) {
            if (combine_discontinuous3_(f1p, last1, d1 - 1, i2, last2, d22, first3, last3, d3, f,
                                        d + 1))
                return true;
            swap(*first1, *i2);
        }
    ...
    if (d != 0) {
        if (d2 > 1)
            rotate_discontinuous3(first1, last1, d1, std::next(first2), last2, d2 - 1, first3,
                                  last3, d3);
        else
...
Function for_each_combination(BidirIter first, BidirIter mid, BidirIter last, Function f)
{
    ...
    detail::combine_discontinuous(first, mid, std::distance(first, mid), mid, last,
...
template <class Int>
inline typename std::enable_if<std::is_unsigned<Int>::value, void>::type check_non_negative(Int d1,
                                                                                            Int d2)
{}

template <class Int>
typename std::enable_if<!std::is_unsigned<Int>::value, void>::type check_non_negative(Int d1,
                                                                                      Int d2)
{
...
{
    return count_each_combination<std::uintmax_t>(std::distance(first, mid),
...
{
    return count_each_permutation<std::uintmax_t>(std::distance(first, mid),
                                                  std::distance(mid, last));
}
...
{
    return count_each_circular_permutation<std::uintmax_t>(std::distance(first, mid),
                                                           std::distance(mid, last));
}
...
public:
    ...
    bool operator()() { return permute(first1_, last1_, d1_, f_); }
};
...
Function for_each_reversible_permutation(BidirIter first, BidirIter mid, BidirIter last, Function f)
{
    ...
    for_each_combination(
        first, mid, last,
        detail::reversible_permutation<Function&, D>(f, std::distance(first, mid)));
    return std::move(f);
}
...
{
    return count_each_reversible_permutation<std::uintmax_t>(std::distance(first, mid),
                                                             std::distance(mid, last));
}
...
template <class BidirIter>
std::uintmax_t count_each_reversible_circular_permutation(BidirIter first,
                                                          BidirIter mid,
                                                          BidirIter last)
{
