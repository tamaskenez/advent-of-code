...
string to_string(pair<Ins::Reg, i64> a)
{
    string s;
    switch (a.FST) {
        case Ins::REG_A:
...
string to_string(Ins& in)
{
    string s = to_cstring(in.opcode);
    s += " ";
    s += to_string(in.arg0);
    if (in.arg1.first != Ins::UNUSED) {
        s += " " + to_string(in.arg1);
    }
    return s;
}
...
Ins string_to_ins(const string& in)
{
    ...
    if (vs[0] == "out") {
        assert(~vs == 2);
        return Ins{Ins::OUT, str_to_val_or_reg(vs[1]), {Ins::UNUSED, 0}};
    }
...
void execute(Machine& m, vector<Ins> ins)
{
    ...
    for (; ~m.clock < 11; ++counter) {
        ...
        // printf("pc %2d: %25s | %lld %lld %lld %lld\n", m.pc, to_string(in).c_str(), m.a, m.b,
        // m.c,               m.d);
        //}
...
int main()
{
    ...
    auto lines = read_lines(f);
    {
        for (int i = 1;; ++i) {
            ...
            m.a = i;
            execute(m, lines);
            bool pattern = m.clock[0] != m.clock[1];
            if (pattern) {
                for (int i = 0; i + 1 < ~m.clock; i += 2) {
                    if (!(m.clock[i] == m.clock[0] && m.clock[i + 1] == m.clock[1])) {
                        pattern = false;
                        break;
                    }
                }
