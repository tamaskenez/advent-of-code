...
struct NIC
{
    deque<Package> input_queue;
    Machine m;
    explicit NIC(VI64 mem) : m(move(mem)) {}
    bool idle = false;
};

struct OtherData
{
    bool first_nat_package = true;
    i64 natx, naty;
    bool all_idle = false;
    optional<i64> naty_sent_last;
    set<i64> naty_sent;
};

void loop_nics(vector<NIC>& nics, OtherData& od)
{
    VI64 out;
    bool need_input;
    VI64 in;
    FOR (nic_ix, 0, < ~nics) {
        auto& nic = nics[nic_ix];
        bool first = true;
        while (first || !nic.input_queue.empty()) {
            first = false;
            if (nic.input_queue.empty()) {
                in = {-1};
                nic.idle = true;
            } else {
                auto p = nic.input_queue.front();
                nic.input_queue.pop_front();
                in = {p.x, p.y};
            }
            tie(out, need_input) = run_until_input_needed_or_halt(nic.m, in);
            assert(need_input);
            if (!out.empty()) {
                nic.idle = false;
            }
            for (int i = 0; i < ~out;) {
                ...
                auto y = out[i + 2];
                if (to == 255) {
                    if (od.first_nat_package) {
                        od.first_nat_package = false;
                    }
                    printf("Package to 255: %lld %lld\n", x, y);
                    od.natx = x;
                    od.naty = y;
                } else {
                    auto p = Package{nic_ix, x, y};
                    nics[to].input_queue.push_back(p);
                    nics[to].idle = false;
                }
                i += 3;
            }
    ...
    if (od.all_idle) {
        nics[0].input_queue.emplace_back(Package{255, od.natx, od.naty});
        printf("all idle, sending %lld %lld to 0\n", od.natx, od.naty);
        if (od.naty_sent_last) {
            if (*od.naty_sent_last == od.naty) {
                printf("Twice in a row: %lld\n", od.naty);
            }
        }
        od.naty_sent_last = od.naty;
        if (od.naty_sent.count(od.naty) > 0) {
            printf("naty sent twice %lld\n", od.naty);
        } else {
            od.naty_sent.insert(od.naty);
        }
    }
}
...
void solve()
{
    ...
    nics.reserve(50);
    FOR (i, 0, < 50) {
        nics.emplace_back(main_input);
        VI64 in = {i};
        VI64 out;
        bool need_input = false;
        tie(out, need_input) = run_until_input_needed_or_halt(nics.back().m, in);
        assert(need_input);
        assert(out.empty());
    }
    OtherData od;
